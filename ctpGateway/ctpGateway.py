...
class CtpGateway(VtGateway):
    ...
    def sendOrder(self, orderReq,strategy=None):
        """发单"""
        return self.tdApi.sendOrder(orderReq,strategy)
...
class CtpTdApi(TdApi):
	...
	if self.times >= 3:
	    if self.curMinute == 0 :
		log = VtLogData()
        	log.logContent = u'连接交易服务器失败！请一分钟后再试！'
	        self.gateway.onLog(log)
	        self.curMinute = datetime.now().minute
	    elif self.curMinute != datetime.now().minute:
		self.times = 0
		self.curMinute = 0
	    return
        log = VtLogData()
...
#========================================================================================
	...
	for x in dic.keys():
	    ...
	    if vtSymbol not in accountID.keys():
	        symbol = {}
	        s = {}
	        l = {}
		s['td'] = 0
		...
		symbol['long'] = l
		symbol['short'] = s
		accountID[vtSymbol] = symbol
	    if dic[x].direction == u"多":

		accountID[vtSymbol]['long']["td"] = dic[x].todayPosition
		accountID[vtSymbol]['long']["ytd"] = dic[x].ydPosition

	    elif dic[x].direction == u"空":
...
#		    accountID[x][y]['ytd']=0
	...
	with open(fileName,'w') as f:
	    f.write(d1)
	    f.close()
    def getPosBuffer(self):
	return self.posBufferDict
#========================================================================================
#========================================================================================
#========================================================================================
    #----------------------------------------------------------------------
