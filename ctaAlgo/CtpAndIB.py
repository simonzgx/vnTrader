...
#2016/11/21=============================================================
	...
	if self.isStop:
		...
		x = len(self.buyPrice)-1-i
	        if self.dfr_2 <= self.buyPrice[x] - self.stpProfit and self.postoday[self.shortsymbol]> x*self.shortBuyUnit :	
		    if i in self.shortCheckList:
			logs = u'策略' + self.name + u'在1秒内重复开单 ' + self.shortsymbol + u'停止运行！'
			self.isStart = False
			self.writeCtaLog(logs)
...
class ParamWindow3(QtGui.QWidget):
	...
	try:	
	    self.paramters["stpLos"] = int(self.lineEdit_label_stpLos.text())
	except ValueError:
	    reply = QtGui.QMessageBox.question(self, u'ERROR!',
	...
	if self.longMKT.isChecked():
	    self.paramters['longMKT'] = True
	else :
	    self.paramters['longMKT'] = False

	if self.shortMKT.isChecked():
	    self.paramters['shortMKT'] = True
	else :
